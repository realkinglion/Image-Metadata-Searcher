...
1. **「画像メタ情報検索くん_v5.1.exe」をダブルクリック**
...
- **既存のv2.0とは別の新しいフォルダ**にv5.1を配置
- **requirements.txt** ファイルも含めて配置
...
#### Step 3: 設定の復元
1. 既存の search_history.json と favorites.json をv5.1フォルダにコピー
2. アプリケーションを再起動
3. お気に入り設定を確認・調整
...
2. **conda環境での解決**:
   ```cmd
   conda install pillow-simd -c conda-forge
   ```
...
**対処法**:
1. **アプリケーションの完全終了確認**
2. **タスクマネージャーでプロセス確認・終了**
3. **metadata_cache.dbファイルを削除**（キャッシュ再構築）
4. **ウイルス対策ソフトの例外設定追加**
...
### Q2: requirements.txtとは何ですか？どう使いますか？
**A**: Pythonプロジェクトの依存関係を定義するファイルです。
...
**内容**:
```
Pillow-SIMD>=9.0.0  # 高速画像処理ライブラリ
exifread>=3.0.0     # EXIF情報読み取り
watchdog>=2.1.0     # ファイル監視機能
```

**トラブル時の対応**:
- エラーが出た場合は個別インストールを試す
- Pillow-SIMD が失敗したら通常のPillowで代替

### Q3: Pillow-SIMDがインストールできません
**A**: 通常のPillowで代替可能です：
```cmd
pip uninstall Pillow-SIMD
pip install Pillow>=9.0.0
